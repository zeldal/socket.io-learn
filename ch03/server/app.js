...
io.adapter(socketIoRedis({host:"localhost", port:6379,db:3}));

var redisStoreIns = new RedisStore({host: "localhost", port: 6379, db: 2});
...
io.use(function(socket, next){
    ...
    if (socket.request.headers.cookie) {
        ...
        if (_cookies['sid']) {
            ...
            return redisStoreIns.get(_sid, function(err, session) {
                if (err || !session) {
                ...
                session.save = function(fn) {
                    redisStoreIns.set(_sid, this, fn || function() {});
                };
